...
画布配置中的 Key 选择器读取当前凡人账号的 Key 列表，支持选择多个 Key，只展示 Key 名称、脱敏 Key、分组和状态，并按选择顺序保存优先级。画布会分别拉取每个 Key 对应分组的模型目录，用户提交请求时只发送当前模型匹配出的 `token_id`：

1. 凡人主站校验 Key 属于当前登录用户。
2. 主站复用既有 TokenAuth、订阅、境界倍率、自动路由、灵石扣费和日志链路。
3. 画布不接触、不保存、不通过 URL 传递 Key 明文。

请求模型会优先匹配明确支持该模型的已选 Key；无法从目录确定时，按 Key 选择顺序使用第一个可用 Key。Key 的模型限制也会在模型目录中应用，避免把文本请求发到只开放生图或视频的 Key。
