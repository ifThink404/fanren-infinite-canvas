...
3. 画布配置弹窗从主站读取当前用户的 Key 列表，支持选择多个 Key，并展示 Key 名称、脱敏 Key、分组和状态；系统按选择顺序保存优先级，并为每个 Key 拉取模型目录。提交每个请求时只发送匹配的 `token_id`，不让用户手填 Key，也不把 Key 明文放进 URL。
...
X-Fanren-Token-ID: <当前请求匹配的token_id>
```

主站会再次校验 `token_id` 必须属于当前用户，然后在内部复用原有的 TokenAuth、订阅和计费链路。模型选择以多个 Key 的模型目录并集为基础，请求优先选择明确支持该模型的 Key，若目录没有记录则按用户选择顺序兜底。任何客户端都不应绕过这个校验直接提交其他用户的 Key。
